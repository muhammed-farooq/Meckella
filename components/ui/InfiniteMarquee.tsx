...
export function InfiniteMarquee({ items }: InfiniteMarqueeProps) {
  ...
  return (
    <div className="w-full overflow-hidden bg-background border-y border-white/5 py-4 flex flex-nowrap items-center">
      <motion.div
      ...
      >
        {[...items, ...items, ...items, ...items].map((item, index) => (
          <div key={index} className="flex items-center gap-12 sm:gap-24">
            <span className="text-accent-gold uppercase tracking-[0.2em] text-xs sm:text-sm font-sans flex items-center justify-center">
              {item}
            </span>
