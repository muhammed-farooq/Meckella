...
export function ProductList({ products }: { products: any[] }) {
  ...
  return (
    <div className="w-full">
      ...
      <div className="border-y border-white/5 bg-background/90 backdrop-blur-md sticky top-24 z-30 py-4 mb-12">
        <div className="max-w-7xl mx-auto px-[20px] lg:px-[70px] flex flex-col md:flex-row justify-between gap-6 items-center">
          ...
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 w-full md:w-auto">
            {filterOptions.map((filter) => (
              <button
                ...
                className={`uppercase tracking-widest text-[10px] sm:text-xs px-3 sm:px-4 py-2 transition-all duration-300 border ${activeFilter === filter
                  ? "border-[#C9A96E] text-accent-gold bg-[#C9A96E]/10"
                  : "border-transparent text-[#A1A1A1] hover:text-foreground"
                  }`}
              >
                {filter}
              </button>
          ...
          <div className="relative w-full md:w-64 shrink-0">
            ...
            <input
              ...
              className="w-full bg-transparent border-b border-white/20 pl-10 pr-4 py-2 text-sm text-foreground placeholder:text-[#A1A1A1] focus:outline-none focus:border-[#C9A96E] transition-colors"
            />
          </div>
      ...
      <div className="px-[20px] lg:px-[70px] max-w-7xl mx-auto w-full pb-24 min-h-[50vh]">
        {filteredProducts.length === 0 ? (
          <div className="text-center py-20">
            ...
            <button onClick={() => { setSearchQuery(""); setActiveFilter("All"); }} className="mt-6 text-accent-gold uppercase tracking-widest text-xs border-b border-[#C9A96E] pb-1 hover:text-foreground transition-colors">
              Clear Filters
            </button>
          </div>
        ) : (
