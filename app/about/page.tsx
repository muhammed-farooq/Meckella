...
export default async function AboutPage() {
  ...
  return (
    <div className="w-full flex flex-col min-h-screen bg-background">
      ...
      <section className="relative min-h-[50vh] md:min-h-[60vh] flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0 bg-[#1A1A1A]">
          {headerBg ? (
            ...
            <div className="w-full h-full bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-[#C9A96E]/10 via-background to-[#0B0B0B]" />
          )}
          <div className="absolute inset-0 bg-gradient-to-t from-background via-background/50 to-transparent z-10" />
        </div>
        <div className="relative z-20 text-center px-[20px] max-w-4xl mx-auto pt-16 md:pt-24">
          <span className="text-accent-gold uppercase tracking-widest text-xs mb-4 md:mb-6 block">Our Heritage</span>
          <h1 className="font-serif text-4xl md:text-5xl lg:text-7xl text-foreground">{pageTitle}</h1>
        </div>
      </section>

      {/* Brand Philosophy */}
      <section className="py-24 lg:py-32 px-[20px] lg:px-[70px] max-w-5xl mx-auto text-center">
        <h2 className="font-serif text-3xl md:text-5xl text-foreground mb-12 leading-relaxed italic opacity-90">
          {philosophyHeading}
        </h2>
        ...
        <div className="max-w-[1400px] mx-auto px-[20px] lg:px-[70px]">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div className="order-2 lg:order-1 flex flex-col gap-8">
              <span className="text-accent-gold uppercase tracking-[0.25em] text-xs">The Sourcing</span>
              <h3 className="font-serif text-4xl lg:text-5xl text-foreground">{ingredients.heading}</h3>
      ...
      {/* The Promise Section */}
      <section className="py-24 lg:py-32 px-[20px] lg:px-[70px] max-w-5xl mx-auto text-center">
        <span className="text-accent-gold uppercase tracking-[0.25em] text-xs mb-6 block">Our Commitment</span>
        <h2 className="font-serif text-3xl md:text-5xl text-foreground mb-8 leading-relaxed">
          {promise.heading}
        </h2>
        <p className="text-[#A1A1A1] text-lg leading-loose font-sans max-w-3xl mx-auto">
          {promise.text}
      ...
      <section className="py-32 text-center px-[20px] bg-[#0f0f0f] border-t border-white/5">
        <h3 className="font-serif text-4xl text-foreground mb-10">Discover Your Signature</h3>
        <Link href="/products">
