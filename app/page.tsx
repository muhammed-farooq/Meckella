  ...
  return (
    <div className="flex flex-col w-full">
      ...
      <section className="relative min-h-[75svh] md:min-h-[80svh] lg:min-h-[85vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-background/40 via-transparent to-[#0B0B0B] z-10" />

        <div className="relative z-20 flex flex-col items-center text-center px-[20px] max-w-5xl mx-auto pt-16 md:pt-24">
          <SplitText
            text={heroSubheading}
            className="text-accent-gold uppercase tracking-[0.3em] text-xs sm:text-sm md:text-base mb-4 md:mb-6"
            delay={1}
          />
          <SplitText
            text={heroHeading}
            className="font-serif text-3xl sm:text-4xl md:text-6xl lg:text-7xl text-foreground mb-6 md:mb-8 leading-tight"
            delay={2}
          />
          ...
          <div className="opacity-0 animate-fade-in-up" style={{ animationDelay: '1.5s', animationFillMode: 'forwards' }}>
            <Link href="/products">
              <Button size="lg" variant="outline" className="px-12 rounded-none uppercase tracking-widest text-sm border-[#C9A96E]/50 text-foreground hover:bg-[#C9A96E] hover:text-[#0B0B0B] transition-all duration-500">
                Explore Collection
              </Button>
            </Link>
          </div>
            ...
            {promotionalBanners.map((banner: any, idx: number) => (
              <Link href={banner.linkUrl || "/products"} key={idx} className="relative group overflow-hidden block aspect-square bg-[#1a1a1a]">
                ...
                <div className="absolute inset-0 z-20 flex flex-col items-center justify-center text-center p-8">
                  {banner.subtitle && (
                    <span className="text-accent-gold uppercase tracking-[0.3em] text-xs mb-4">{banner.subtitle}</span>
                  )}
                  <h3 className="font-serif text-4xl lg:text-5xl text-foreground group-hover:scale-105 transition-transform duration-700">{banner.title}</h3>
                  <div className="mt-8 flex items-center justify-center opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500">
                    <span className="text-xs uppercase tracking-widest text-foreground border-b border-[#C9A96E] pb-1">Shop Now</span>
                  </div>
                </div>
              </Link>
            ))}
        ...
        <section className="py-16 md:py-[120px] bg-[#0f0f0f] border-t border-white/5">
          <div className="max-w-[1400px] mx-auto px-[20px] lg:px-[70px]">
            <div className="mb-10 md:mb-20">
              <h2 className="font-serif text-4xl sm:text-5xl text-foreground mb-6">A Study in Atmosphere</h2>
                ...
                const content = (
                  <>
                    ...
                    <div className="absolute bottom-0 left-0 p-8 z-20 w-full">
                      <h3 className="font-serif text-3xl text-foreground mb-3 group-hover:text-accent-gold transition-colors">{mood.title}</h3>
                      <p className="text-[#A1A1A1] text-sm max-w-sm group-hover:text-foreground transition-colors">{mood.description}</p>
                    </div>
                    {mood.productSlug && (
                      <div className="absolute top-6 right-6 z-20 opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500">
                        <span className="bg-background/80 backdrop-blur-sm border border-white/10 text-accent-gold text-[10px] uppercase tracking-widest px-4 py-2 hover:bg-[#C9A96E] hover:text-[#0B0B0B] transition-colors">Discover Fragrance</span>
                      </div>
                    )}
                  </>
                );
      ...
      {craftsmanship.length > 0 && (
        <section className="py-16 md:py-[120px] lg:py-[160px] px-[20px] lg:px-[70px] max-w-5xl mx-auto w-full">
          <div className="text-center mb-12 md:mb-24">
            <span className="text-accent-gold uppercase tracking-[0.3em] text-xs mb-6 block">The Process</span>
            <h2 className="font-serif text-4xl sm:text-5xl text-foreground mb-6">Tradition & Innovation</h2>
          </div>
          ...
          <div className="flex flex-col gap-10 md:gap-16 relative before:absolute before:inset-0 before:ml-[23px] md:before:mx-auto md:before:w-px before:h-full before:bg-[#333]">
            {craftsmanship.map((item: any, idx: number) => (
              <div key={idx} className={`relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group w-full`}>
                {/* Node */}
                <div className="absolute left-0 md:left-1/2 w-12 h-12 rounded-full border border-[#333] bg-background text-accent-gold flex items-center justify-center font-serif text-sm z-10 -translate-x-0 md:-translate-x-1/2 group-hover:border-[#C9A96E] transition-colors duration-500">
                  {item.step}
                </div>
