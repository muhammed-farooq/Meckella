...
export default async function ProductDetailsPage({ params, searchParams }: { params: any, searchParams?: any }) {
  ...
  if (!product) {
    return (
      <div className="w-full flex flex-col min-h-[70vh] bg-background text-foreground items-center justify-center pt-24 px-[20px]">
        <h1 className="font-serif text-4xl mb-6">Product Not Found</h1>
        <p className="text-[#A1A1A1] max-w-lg text-center leading-relaxed">
          We couldn't locate tracking details for <span className="text-accent-gold">{resolvedParams.slug}</span>.
          Please make sure the product is fully <strong>Published</strong> in your Sanity Studio, and not just a Draft!
        </p>
        <Link href="/products" className="mt-8">
          <Button variant="outline" className="uppercase tracking-widest text-sm px-8 border-[#C9A96E] text-accent-gold">
            Back to Collection
          </Button>
        </Link>
      </div>
  ...
  return (
    <div className="w-full flex flex-col min-h-screen bg-background text-foreground">
        ...
        <div className="flex flex-col pt-12 lg:pt-0">
          <div className="mb-10">
            ...
            {product.emotionalSection && (
              <p className="text-accent-gold font-serif text-xl italic tracking-wide">
                "{product.emotionalSection}"
              </p>
            )}
          </div>
          ...
          <div className="mb-12">
            <a href="#how-to-use" className="inline-flex items-center gap-2 text-accent-gold hover:text-foreground transition-colors border-b border-[#C9A96E] hover:border-[#EDEDED] pb-1 uppercase tracking-widest text-xs">
          ...
          {product.scentProfile && product.scentProfile.length > 0 && (
            <div className="mb-12">
              <span className="text-foreground uppercase tracking-widest text-xs mb-4 block">Scent Profile</span>
          ...
          {(product.topNotes || product.middleNotes || product.baseNotes) && (
            <div className="mb-12 border border-white/10 p-8 relative overflow-hidden bg-[#0f0f0f]">
              ...
              <h3 className="font-serif text-2xl text-foreground mb-8">Fragrance Pyramid</h3>

              <div className="flex flex-col gap-6">
                {product.topNotes && product.topNotes.length > 0 && (
                  <div className="flex gap-4">
                    <div className="w-24 shrink-0 text-accent-gold uppercase tracking-widest text-xs pt-1">Top Notes</div>
                    <div className="text-[#A1A1A1] font-sans">{product.topNotes.join(" • ")}</div>
                  </div>
                )}
                {product.middleNotes && product.middleNotes.length > 0 && (
                  <div className="flex gap-4">
                    <div className="w-24 shrink-0 text-accent-gold uppercase tracking-widest text-xs pt-1">Heart Notes</div>
                    <div className="text-[#A1A1A1] font-sans">{product.middleNotes.join(" • ")}</div>
                  </div>
                )}
                {product.baseNotes && product.baseNotes.length > 0 && (
                  <div className="flex gap-4">
                    <div className="w-24 shrink-0 text-accent-gold uppercase tracking-widest text-xs pt-1">Base Notes</div>
          ...
          {product.keyHighlights && product.keyHighlights.length > 0 && (
            <div className="mb-12">
              <span className="text-foreground uppercase tracking-widest text-xs mb-4 block">Highlights</span>
      ...
      {product.amazonLink && (
        <div className="fixed bottom-0 left-0 w-full z-50 p-4 md:p-6 bg-gradient-to-t from-background to-transparent pointer-events-none flex justify-center md:justify-end">
